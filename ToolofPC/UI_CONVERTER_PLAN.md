...
- [ ] 载入 `samples/` 五张卡，UI 都能正确渲染
- [ ] 多份 UI 的卡能用 chip 切换（`灰港迷雾` / `织_夜航酒保` / `织房夜话` 各 3 份）
- [ ] 与主项目 Assembly 编辑器的运行时预览外观一致
- [ ] 工作区「文本 / UI」标签切换正常，逐卡独立

**离线已验证**：五张卡的提取逻辑全部通过
（scene 4页/93件、3页/94件；灰港迷雾 3 份；织房夜话含 extra_companion）。
