...
### 阶段 3：UI 生成（新流水线步骤）🟡 **方案A 完成，缺陷补到 P1**
...
**P0-2 开场白净化**（`greeting_sanitizer.dart`）

三级策略，**保守优先**：

| 情况 | 处理 |
|---|---|
| 有 `<正文>`/`<content>` 等正文标签 | 只取那一段 |
| 无正文标签但有已知标签 | 删已知标签块 + onclick 选项块 |
| 完全没标签 / 标签不认识 | **原样返回** |

最后一条是关键：**认不出来就不动**。宁可留几个尖括号，
也不要把作者写的正文误删——后者不可逆。
删完为空时也会退回原文。

**P1-1 每个开场都标 keyAction**

各分支平级，点哪个都该「选定并关闭」，
不存在「第一个特殊、其余普通」。

顺带修正了校验器的一条过时规则：它原本警告「同一页只该有一个
keyAction」。查证运行时用的是 `isKeyAction(module)` **逐个判断**
（见 `ui_assembly_runtime_view.dart` 的 tap 分支），
不是找唯一那个——**并列多个完全合法**。
现在只对非 opening 的 mode 提醒。

**P1-2 按压反馈**

每个选项配一条 `click_to_surface_press` 联动器，
放在 PCB 外的后台位（负 x 坐标，不占版面也不参与包含性校验）。

button 是不显形的热区，不接联动器点下去毫无反馈。
...
| 1 · 预览面板 | ✅ | 样卡全部正确渲染 |
| 2 · 契约与校验 | ✅ | 变异测试 9/9，契约机器导出 |
| 3 · UI 生成 | 🟡 | 方案A + 分支体系已通；余 P1-3 / P2（见 3d） |
| 4 · 自检扩展 | ⬜ | |
| 5 · AI 设计工作台 | ⬜ | |
