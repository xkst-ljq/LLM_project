...
void main() {
  ...
  group('UiAssemblyBuilder - 分支动作区', () {
    test('动作区生成 status field，branch_initial_values 存各分支动作', () {
      ...
      // 动作 status field 应存在（动作1、动作2），type=text
      final actionFields =
          built.statusFields.where((f) => f['id'].toString().startsWith('sf_act_')).toList();
