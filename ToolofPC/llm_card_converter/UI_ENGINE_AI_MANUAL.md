...
*   **常驻气泡面板 (Companion UI)**：
    *   物理宽度硬性约束在 **`212px` (极限宽度)**。为了兼顾视觉的极简舒展和多维丰富属性，**自动采用多页签多页面（Base Pages & Tab Bar）联动结构**。
    *   **Tab-bar 导航**：在顶部生成三个等宽 Tab：`📊 属性`、`📁 档案`、`🎯 选项`。
    *   **多页分流**：
        *   第一页：`📊 属性` ➔ 高对比度自适应单列展示属性进度条（生命红、精神紫、体力绿、饱腹橙），并装饰 Emojis。
        *   第二页：`📁 档案` ➔ 纯文本单列最宽敞舒展地展示称号、编号、罪名等信息，防止横向挤压。
        *   第三页：`🎯 选项` ➔ 集中放置该卡片提取到的所有静态快速选项（onclick actions），让玩家在后续所有轮次中能随时翻回气泡进行点击，完美解决选项点击一次即滚走、后续无法触达的痛点。
    *   **页签高度对齐**：计算三页元素的最大物理外框（`pcbH`），统一各底板尺寸，确保换页时面板大小恒定。
