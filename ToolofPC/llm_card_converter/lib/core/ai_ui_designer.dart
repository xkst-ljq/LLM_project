...
- **画布宽度恒为 360**（横向设计坐标系）。所有元素的 x + width 必须落在
  0~360 内，绝不允许超过 360 或为负。这是绝对的硬边界，不是建议。
...
【建立画布心智：像在 360 宽的手机竖屏上排版】
把每个页面想象成一张 360 宽的手机屏幕，从上到下分段排：
- 左右两列：x 用 14 和 ~180 两档（两列各 ~166 宽，中间留 ~12 间距）；
- 单行通栏：x=14，width≈332；
- 靠右的小按钮（如设置）：x≈300，width≈44。
- 元素之间的纵向间距：同组字段 y 递增 26~40；不同区块之间留 50~70。
- **始终在心里检查**：x+width 有没有超过 360？这个元素会不会盖住上面/下面的元素？
...
- **x 是元素左缘坐标，width 是宽度，必须满足 x≥14 且 x+width≤360**。
- y 是元素顶缘坐标，height 是高度，尽量满足 y+height ≤ pcbHeight。
- 想横向排布/两列：给不同的 x（如 14 和 180）+ 合适 width（各 ~166）即可。
- 用字段的 y 控间距，x + width 控横向布局。
- **给出坐标后，在心里复核一遍：有没有越界（x+width>360 / y+height>页高）？
  有没有两个元素矩形相交（重叠）？** 这两点是 AI 最常见的错误，请自我检查后再输出。
- 不给 x/y/w/h 时，Step B 会自动纵向排布（兜底，不会越界）。
