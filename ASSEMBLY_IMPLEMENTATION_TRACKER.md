...
作者会觉得规则是随机的。测试里有一组断言专门锁这个等价性。

**换父执行体也共用**：`_applyPageReparent` 同时服务弹窗与拖放。
两份实现迟早漂移——`sortOrder` 重排这种事最容易只改一边。

**防环**：不能挂到自己的后代下面。成环不会死循环
（`_pageDepth` 有 visited 兜底），但那一支会从树上消失。
...
测试：`test/assembly_page_reparent_drag_test.dart`（30 例），
含「拖放判据 == 弹窗候选」等价性组与四条防环断言。
