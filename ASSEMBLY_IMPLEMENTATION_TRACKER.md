...
| A10 | mode 差异逻辑收口 | A10-5 完成待测；余 A10-3 伴生 | 🚧 |
| A11 | 消息流窗口 | A11-1 骨架 + 可滚动长文本完成并测试通过 | 🚧 |
| A12 | 高级动画 | 未开始 | ⏳ |
...
- 富文本：接上 `_buildMarkdownWidget`，支持 Markdown / HTML
  **消息流与可滚动长文本一起接**（用户要求），避免重复工作。
  readme 场景尤其需要标题 / 加粗 / 列表。
- 重新生成 / 编辑 / 删除消息
- 版本切换（左右箭头）
- 头像显示
...
```
1. 背景图层
2. 消息列表 + 状态栏   ← scene 接管时 IgnorePointer
3. 常驻 UI            ← scene 接管时不渲染
4. 输入栏             ← scene 接管时不渲染
5. scene 场景 UI      ← 全屏铺满
6. 开场白 UI          ← 仍在最上（先看开场白，确认后进场景）
```
常驻 UI 一并隐藏：场景已是完整界面，再浮一个挂件既遮挡又语义重复。
...
### 待补的三件事
1. **结构化档案条目**：现在只能散装变量，需要支持一组字段自成一段注入：
   ```
   [玩家档案]
   姓名：林
   职业：剑士
   力量 8 / 敏捷 6 / 智力 4
   ```
2. **注入位置可选**：数据通道现在只能进 `[界面数据]`，
   需要让作者指定「这条进核心设定区」。
3. **点数分配玩法**：多个 slider 共享一个总额配额的约束逻辑。
