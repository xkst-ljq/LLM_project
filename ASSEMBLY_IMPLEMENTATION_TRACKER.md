...
| A11 | 消息流窗口 | 全部完成并测试通过 | ✅ |
| A12 | 高级动画 | 六种动画全部完成并测试通过 | ✅ |
| A13 | 玩家参与的会话初始化 | A13-1/2/3 全部完成并测试通过 | ✅ |
| A14 | Assembly 编辑器深层交互 | A14-1~5 全部完成并测试通过 | ✅ |
| 4.1 | 拖出 PCB 自动后台化 + 三态边界约束 | 完成并测试通过 | ✅ |
| 4.2 | 叠加页拖放换父（平级页之间切换） | 完成并测试通过 | ✅ |
| 4.3 | HUD：数据通道页重构 | 完成并测试通过 | ✅ |
| — | 状态栏 ⇄ UI 初始值双向同步 | 完成并测试通过 | ✅ |
| 4.3b | 状态变化通知系统（浮窗 / 弹窗） | 完成并测试通过 | ✅ |
| — | 聊天页启动/渲染性能整治（背景预热、键盘避让、重建收敛） | 完成并测试通过 | ✅ |
| — | `button_to_page_route` 运行时消费方（原缺口） | 完成并测试通过 | ✅ |
| — | 复合件入库：导入自动收录 + 指纹去重 + 净化成模板 | 完成并测试通过 | ✅ |
| — | 示例卡扩充至 5 张 + `validate_card.py` 结构校验器 | 完成并测试通过 | ✅ |
...
## A14：Assembly 编辑器深层交互（✅ A14-1~5 全部完成，测试通过）
...
用户观察：端口位置会跟着元件转，但**接线的出头依旧水平拉取**。

确认是真缺陷。painter 里两个控制点写死了水平方向：

```dart
final cp1 = Offset(start.dx + controlOffset, start.dy);  // 恒定朝右
final cp2 = Offset(end.dx - controlOffset, end.dy);      // 恒定朝左
```

端口位置按旋转算了，切线方向没算，所以元件转 90° 时线会先横着窜出去
再拐回来，看着像从元件侧面漏出来的。

修法：给画笔加 `startDirection` / `endDirection` 两个方向参数
（默认 `(1,0)` / `(-1,0)`，与历史行为一致），
由调用方用 `rotateDirection(base, el.rotation)` 算出朝向。

连带修掉一个隐藏问题：`controlOffset` 原本只取 `|dx| * 0.4`，
那是「切线恒为水平」时的合理近似。端口朝向变化后两端可能垂直对齐，
dx 趋近 0 会让曲线塌成直线段。改为**仅在两端都是默认水平方向时**
沿用旧公式（保证 Studio 观感零变化），其余情况用两点距离。

**Studio 一并修**（同样支持旋转元件，缺陷相同）：
已有连线与拖拽临时线都传入方向。临时线的自由端（跟着手指、
尚未吸附）用 `_freeEndDirection` 取「指回起点」的方向，
固定朝左的话往左拖会画出多余回勾。
复合件内部子元素以外框为锚点、不参与旋转，仍按默认水平处理。

Gate 的新旧公式已验证等价：
`Offset(end.dx, end.dy - k)` ≡ `end + (0,-1) * k`。

`shouldRepaint` 补上方向比较——元件绕中心转到对称位置时端点可能不变，
只有方向变了，不比较就会漏重绘、线卡在旧朝向。

测试增至 23 例，新增「端口切线跟随旋转」组。

**附带修复：Studio 旋转后把手失效**

用户反馈「组件旋转一定角度后把手就失效，是不是容器没跟着旋转」。

**不是容器的问题——容器确实转了。** 真凶是包在
`Transform.rotate` **外层**的 `IgnorePointer`：

```dart
IgnorePointer(              // ← 外层，问题所在
  child: Transform.rotate(child: rootTree),
)
```

`RenderTransform.hitTest` 特意不检查自身尺寸，直接把落点逆变换后
交给子树；而 `IgnorePointer` 是普通 `RenderProxyBox`，会做
`_size.contains(position)` 这一步**轴对齐**边界检查。
它在外层，检查就发生在旋转**后**的坐标系里，size 却仍是未旋转的 W×H。

实测 240×100 的节点，把手在右下角：**15° 就已出界**，
与用户「旋转一定角度后」的描述吻合。元件本体还能点，
是因为本体大部分仍在盒内，只有伸在角上的把手最先出界。

改法：`IgnorePointer` 挪进 `Transform.rotate` 内层，
边界检查回到旋转前的局部坐标系，`ignoring` 语义不变。

关于用户说的「以前修好过」：查了 git 历史，本仓库 105 个提交里
`ui_studio_page.dart` 只被动过两次，没有修复痕迹。
推测当年修的是「元件本体点不到」，把手这个更靠边的命中点未暴露。

**Assembly 侧无此问题**：它的 `IgnorePointer` 都在 rotate 内层，
且尚无把手（A14-1g 推后）。做把手时这条约束正好用得上。
根因与排查口诀已写进 HANDOFF。

#### A14-4 第二步：拖拽画线交互（已完成，待本地测试）

用户选定的三项参数：
- 热区**左右各 24px**（Studio 是 32px）
- 冲突处理**照搬 Studio**：`_isDraggingConnection` 为真时所有 pan 提前 return
- 方案选择**两端齐了才弹**

**热区为什么收窄。** Assembly 的 linker 是 132×44，切 32 之后中间
只剩 68px，手指拖动元件时容易偏进热区。24px 仍明显宽于手指接触宽度，
接线不难点，中间留 84px 好按。
...
pan 要先跨过 kTouchSlop 才触发，那段距离里手指已经移开、起点会不准，
而且会与画布平移进入手势竞技场互相抢。

**接线的移动/抬起挂在最外层 Listener**，不是挂在 linker 上——
手指一旦离开 linker 本体，挂在它身上的监听就收不到事件了。

**三处 pan 让路**（照搬 Studio 的隔离方式）：
画布平移 `onPanUpdate`、linker 中间区的元件拖动、`onPanEnd` 的持久化。
资产栏拖放 `_handlePlacementPointerMove2` 本就由 `_activePlacement`
把关，不会冲突，但仍在最外层做了分流。

**落点只写被拖的那一端**（左侧写 `sourceModuleId`、右侧写
`targetModuleId`），另一端保持原样——作者常常是在改接一条已配好的连线。

**两端齐了才弹方案选择**，且与 Studio 一样先查方案矩阵：
若这对类型之间没有可用方案，自动断开并提示。
留一条永远不会生效的连线只会让作者困惑。

**双击某侧热区断开该端**，并把 `scheme` 置回「未配置」——
少了一端方案必然不成立，留着会造成「显示已配置但不生效」。
断开左侧时连 `sourceGesture` 一起清（A14-3 的连线级手势挂在这一端）。

**全锁校验三处**：起拖时、落点时（检查另一端）、双击断开时。
与配置式对话框同一条规则，全锁的语义就是「连线不可改」。
全锁元素也不能作为拖拽命中的目标。

**新增端口提示点**：linker 两侧各一个小圆点，实心=该端已连、
空心=未连。作者不必打开对话框就能看出「这条还差哪一端」。

命中检测留 12px 容差，并支持旋转元件（落点逆旋转回局部坐标系再判断）。

新增 `test/assembly_linker_drag_test.dart`（18 例）：
...
#### A14-5 PCB 自定义（已完成，待本地测试）

**盘点发现一个真缺口。** `_pcbColor` / `_pcbRounded` 这两个字段，
数据结构、序列化、渲染全都齐了，但**全局没有任何赋值点**，
只在初始化时读一次——PCB 永远是白色圆角，作者根本改不了。
与 A14-3 之前「Assembly 一个取色器都没有」是同一类缺口：字段是死的。

尺寸虽能拖手柄，但拖不出精确值（想要正好 300 宽只能碰运气），
因此一并给出数值输入，与 A14-2 给元件补精确几何同理。

**圆角升级为数值**（用户选定）：
布尔 `pcbRounded` 只能在 20 与 0 之间二选一，做不出「微圆角 8」。
新增 `pcbRadius`（0~40），旧卡按 true→20 / false→0 自动迁移；
`toJson` 仍继续写出布尔字段，让老版本读到新卡时不至于变成直角。

上限取 40 是经验值：PCB 最窄 120，再大的圆角会把四角啃掉太多可用面积。

入口放顶栏（图层按钮左侧），面板含底色（复用 A14-3 的 15 色板）、
圆角滑杆、精确宽高输入。伴生模式额外给一条橙色提示，
说明宽度上限更窄的原因。

**尺寸输入的容错**：空 / 非数字时**保留原值**而非回落默认——
作者手滑清空输入框不该让辛苦拖出来的画布跳回默认尺寸。

新增 `test/assembly_pcb_customization_test.dart`（13 例）：
圆角迁移四种情形、序列化双写与往返、尺寸夹取与容错。

#### 微调键盘反向移植回 Studio（已完成，待本地测试）

用户原话：

> 你做的微调键盘我很喜欢，不仅比 studio 更加紧凑，
> 而且中间的方块还有相对坐标，我建议可以将这个方式同样替换给 studio。

这是**唯一一次 Assembly → Studio 的反向移植**。此前一直是 Assembly
对齐 Studio，这次方向反过来。
...
| | 旧 Studio | 移植后 |
|---|---|---|
| 间距 | 16（总宽 152）| **12（总宽 144）** |
| 中心方块 | 纯装饰空色块 | **显示当前坐标** |
| 代码位置 | 全挤在 build 里 | 抽成 `_buildFineTunePad` |

中心显示坐标不只是好看：它兼作「我正在调的是哪个组件」的确认——
盲点方向键时最容易犯的错就是调错了对象。

坐标语义两边通用：Studio 的 `el.offset` 相对 workspace、
Assembly 的相对 PCB，都是「元素在自己画布里的位置」。

**顺手修了一个居中偏差。** 移植时验算发现实际总宽是
`40×3 + 12×2 = 144`，而我一开始按 136 写、偏移填了 68，
会让整个 D-Pad 偏右 4px。Assembly 侧原本就是 72（正确），
Studio 已同步为 72。

新增 `test/nudge_pad_layout_test.dart`（7 例）守住布局不变量：
三行等宽、占位宽 = 箭头 + 间距、偏移 = 总宽/2。
任何一侧改尺寸而忘了同步，测试会失败。

#### A14-1g 形变把手（已完成，待本地测试）

**范围：只做形变把手，不做旋转把手**（用户选定）。
旋转使用率不高，精确几何里已有角度输入且更准；
Studio 的单把手靠「点一下切换形变/旋转」两用，
代价是每次形变前都要确认当前模式——为低频功能给高频操作上税不划算。

**用户的纠结点指向了一个真 bug。** 用户说
「旋转过后形变把手也会跟着旋转，感觉还不如把旋转把手一并做了」。

核实后发现别扭的真正来源不是「把手跟着转」，而是
**Studio 把屏幕 dx/dy 直接当宽高增量**：

```dart
final newWidth = (_startTouchWidth + dx).clamp(...);   // dx 是屏幕位移
```

元件转 90° 后，它的「宽」轴在屏幕上是竖直方向，
这时朝屏幕右边拖改的却仍是 width，视觉上元件在往下长。
...
投影后把手朝哪个视觉方向，就朝哪个方向拖、元件朝那个方向长，
旋转后反而完全直觉，**不需要靠旋转把手来补偿**。
Studio 与 Assembly 两边都已修（用户选定保持一致）。

**结构问题：Assembly 元件没有 padding。**
Studio 靠 `p = 20` 撑出把手空间，Assembly 的 `Positioned` 是严丝合缝的
`el.size`。这不只是好不好看——Flutter 的 `RenderBox.hitTest` 会先判断
`size.contains(position)`，**画在盒外的把手根本收不到点击**
（与 A14-4 那个 IgnorePointer 把手失效同源）。

解法：选中且可形变时四周各撑 `kResizeHandlePadding = 12`，
内容整体内移 12。四周等量扩张，padded box 的中心仍是元件中心，
因此旋转中心不变；连线端点基于 `el.offset`/`el.size` 计算，
不读 widget 盒子，同样不受影响。

`_buildElementWidget` 拆为「撑开外壳」+ `_buildElementBody`，
避免每个分支都改一遍。

**把手的显示条件**：仅选中、未锁定、且非固定尺寸逻辑件
（linker / math_node / timer / page_router / indicator 不给，
与 A14-1 的几何按钮同口径）。
未选中时 padding 为 0，避免平时白白扩大命中区域互相抢点击。

新增 `test/resize_handle_test.dart`（17 例）：
局部轴投影六种角度、显示条件、尺寸夹取、撑开不影响中心与连线端点。

**首轮测试反馈：旋转后形变没有锚点，图形到处跑（已修）**

用户测完全部条目后指出：**问题不在我列的任何一项里**——
旋转 90° 后拖把手，图形会到处跑，Studio 与 Assembly 都有。

我上一轮验的是「四周等量扩张所以**旋转中心**不变」，
那句话没错，但**形变锚点是另一回事**，完全没被覆盖。

根因：`Transform.rotate` 绕中心旋转，`offset` 却是左上角。
改尺寸时 offset 不动 → 中心移动 → 旋转后绕着移动的中心转。
实测转 90° 加宽 20，左上角在屏幕上跑了 (10, -10)。
未旋转时中心同样在动，只是方向与屏幕轴对齐、看不出来。

修法见 HANDOFF「改尺寸必须同时修正 offset」一节：
让把手对角保持不动，反推 offset，0° 时公式自动退化为不变。

Assembly 逐帧增量修正、Studio 起点快照一次性修正，
已用测试断言两者等价（线性变换可叠加）。
...
**1. 未选中的 linker 拖动被打断，且 Studio 的 linker 没有边框**

用户两条反馈指向同一处修改。Studio 确实不给 linker 画虚线框
（`isTransformationActive && !isLinker`），移除后两个问题一并解决。

打断的根因是「手势进行中改变树结构」的**第三个变体**：
虚线框是条件子节点且**排在手势节点之前**，首次拖动未选中的 linker 时
插入它会使后面所有子节点索引后移一位，GestureDetector 被当成新 widget
重建，pan 随之中断。

其他元件不受影响，是因为它们的 GestureDetector 是最外层包裹；
只有 linker 因三段式布局把手势放在 Stack 内部。
用户「已选中时拖动就正常」的观察正好印证——那时节点已存在、索引不变。

**2. 复合件缩放后边框不贴合容器面**

用户诊断准确：容器面圆角随 `Transform.scale` 缩放，
虚线框却用原始值。实测原始圆角 16 时，放大 1.8 倍差 12.8px、
缩小到 0.45 差 8.8px。修法是虚线框圆角乘上内容缩放比。

用户还问「其他类型的面会不会有同样隐患」——核实结论是**不会**：
只有 `rounded` 读绝对值，其余形状的路径都由 rect 自适应推导。

测试增至 34 例。

#### 移除联动器的来源/目标选择器（已完成，待本地测试）

用户提议：画线做好之后，那两个下拉就是重复入口了。

同意。同一件事有两个入口时作者会困惑「哪个才算数」，
而且下拉列表在元件一多时根本找不到目标；画线是所见即所得的。

改为**只读展示行**：彩色圆点 + 「来源 / 目标」+ 组件名·类型，
未连时显示灰色斜体提示「拖动联动器左侧接出」。
配色沿用 `LinkerLineColors`，与画布上的连线颜色对应。

**移除时发现一个既存 bug。** 旧的「清除连接」按钮靠
「把两个下拉设空 → 保存」工作，但 `canSave` 要求两端非空——
清空后保存按钮立刻变灰、根本提交不了，清除等于失效。

改为独立动作：对话框返回值从 `bool` 改为
`'cancel' / 'clear' / 'save'` 三态，`clear` 直接调用
`_disconnectLinkerBothEnds`，不经过保存校验。
只连了一端时也能清除（旧实现在这种情况下必然失效）。

顺带清理：
* 删除已无引用的 `_linkerCandidates()`
* 保存分支里「两端为空」的清空逻辑成为死代码（canSave 已保证非空），移除
* `sourceId` / `targetId` 改为 `final`
* 新增 `_wiringEndpointById`：端点可能是复合件内部的暴露子元素，
  不在 `_elements` 顶层，只读展示要深查 `composite.children` 才找得到

测试增至 33 例，新增「配置面板移除来源/目标选择器」组，
含「只连了一端也能清除」这条针对旧 bug 的回归断言。
...
| | 内容 | 结论 |
|---|---|---|
| 4.1 | 边缘吸附 / 拖出后台化 | ✅ **已完成**。原判「依赖未就绪」是误判，`runtimePlacement` 机制早已存在，缺的只是 Assembly 入口与手势 |
| 4.2 | 树状拖放换父级 | ✅ **已完成**（原判被推翻）。按页面类型分流手势即可消解冲突：平级页只排序、叠加页只换父 |
| 4.3 | HUD 样式调整 | 有效，但属于最后打磨，跟着实际使用感受调 |
| 4.4 | 统一几何坐标原则 | **本就不是待办，是开发约定**，A14-4 已在遵守 |
| 4.5 | Binding 名称解析 | **已由 A13-2 实现**（`card_entry_target.dart` + targetId/pendingName 双轨）|
| 4.6 | 资产库底栏横向抽屉 | **已实现**（`_buildAssetDock`），条目过时 |

各条目已就地标注状态，避免后续再把已完成 / 已变约定的东西当缺口重做。
...
### A12-1：统一动画通道（已完成，待本地测试）

**动手前的盘点发现一个架构问题**，不先解决的话每加一种动画
都要重复同样的活：

| 问题 | 说明 |
|---|---|
| 每种动画各一套字段 | surface 用 `anim_trigger`/`anim_timestamp`/`anim_duration`/`anim_radius`；indicator 用 `eventFlashTimestamp`/`eventFlashColor`/`eventFlashDurationMs`。再加三种就是五套 |
| 只有 surface 能播 | `_buildAnimatedSurface` 只在 `case 'surface'/'base_box'` 调用。数值跳动目标是 progress/text、发光脉冲用在 indicator/image，旧结构接不上 |
| 参数塞在方案里 | `durationMs` 是 `SchemeParamField`，同一元件被两条连线驱动就得配两遍，还可能配出两个不同的值 |

第三点撞上既定分工线：**动画是「这个元件在这张卡里怎么表现」**，
该归 Assembly 的元件配置，不该散在每条连线里。

**做法**（用户选定「先统一再加」+「参数归元件」）：

* 新增 `services/ui_engine/element_animation.dart`：
  单一字段族 `__anim`（type / durationMs / curve / intensity /
  color / ts），六种类型的 enum + 六条精选曲线
* 渲染层抽 `_wrapWithAnimation`，在 `render()` 里对**所有可见组件**
  生效，不再只有面板；旧 `_buildAnimatedSurface` 删除
* 动画参数迁到外观页（A14-3 腾出的空间），连线只负责触发
* 编辑期不播动画——它依赖真实触发时刻，编辑器里只会看到
  打开页面那一瞬的残留帧，反而干扰排版

**兼容处理**（老卡不能炸）：

* `readFrom` 读取顺序：新 `__anim` → 旧 `anim_*` → 旧 `eventFlash*`
* 旧 `rippleRadius` 是绝对像素（默认 150），换算成 0~1 相对强度，
  元件尺寸变化时不用重配
* `_ensureLegacyAnimationConfig`：老卡元件还没有 `__anim` 时，
  首次触发用方案参数现场补一个，之后不再覆盖作者改过的值
* `writeConfig` / `stamp` 都会清掉两套旧字段，避免两个时间戳打架

**一个特例**：indicator 的 flash 由它自己改灯色实现，
不走通用包裹层——指示灯本体是一颗小圆点，盖半透明蒙版只会让它变浑浊，
直接换灯色才有「亮了一下」的观感。已在 `render()` 里显式跳过，
否则会在改色之上再叠一层同色蒙版。

六种动画已全部实现绘制：按压凹陷、水波扩散、短暂高亮、
数值跳动、发光脉冲、粒子迸发。粒子用固定角度分布而非随机——
`CustomPainter` 每帧重新构造、拿不到上一帧状态，随机会让粒子乱跳。

新增 `test/element_animation_test.dart`（24 例）：
序列化往返、两套旧字段兼容、写入时清理、打戳、播放窗口、类型元数据。

### A12-1 补丁：通用动画触发方案（已完成，待本地测试）

用户提问「该如何连线让进度条或者文本触发动画呢？」——
核实后确认**连不了，这是 A12-1 的缺口**。

渲染层已经做成「所有可见组件都能播」，但**触发端只认三条老方案**：
`event_to_indicator` / `click_to_surface_press` /
`click_to_surface_ripple`，它们的 `targetType` 分别写死是
indicator 与 surface。于是进度条、文本虽然会画动画，
却没有任何一条连线能触发它。

（我上一轮说「给 progress 配数值跳动、连线触发」是错的，已纠正。）

新增 `event_to_animation`：
* 来源 `button` / `timer`
* 目标为全部可见组件（逻辑件排除——不显形，播动画没意义）
* **不带任何动画参数**：播什么、播多久由目标元件的外观页决定，
  方案只回答「什么时候播」。这是 A12 确立的分工，
  方案再带一份 durationMs 就会出现同一元件两个时长的矛盾
* `_schemeTargetPort` 里必须放在通配规则**之前**拦住，
  否则会落到兜底 `'value'`、被运行端当成写数据字段
* 刻意**不给它老卡兜底配置**：A12 之后才有的方案不存在兼容问题，
  作者没配动画就什么都不播

测试增至 29 例。

### A12-2：值变化自动播放（已完成，待本地测试）

补的是 `event_to_animation` 暴露出的形态问题：数值跳动的自然语义是
**「进度条的值变了，它自己弹一下」**，不该还要额外接一条连线——
那等于让作者手动告诉系统「现在数值变了」，而系统自己明明知道。

**关键约束：不能在渲染时写 props。**
最直接的做法是渲染时比较新旧值、变了就打时间戳，
但这违反「渲染函数只读不写」（HANDOFF）：写进 props 的时间戳会被
`_persistAssemblyElements` 存进角色卡，既污染产物，
又会让作者下次打开时凭空看到一次动画。配额归零正是这么踩过一次。

改用 `ValueChangeAnimator`（StatefulWidget）的**本地状态**记住上一次的值，
...
| 维度 | 范围 | 理由 |
|---|---|---|
| 组件类型 | progress / text / slider / select / input | 面板、按钮没有「值」可言 |
| 动画类型 | numberPop / glowPulse / flash | 按压、涟漪是「我碰了它」的交互反馈，值变了自己涟漪一下会很怪 |

**取值必须取解析后的值**（`LinkerService.resolveTargetValue`）
而非 `properties['current']`——联动驱动的组件属性里存的是初始值，
比较原始属性永远检测不到变化。

首帧不播：刚进页面时所有值都是「从无到有」，全部弹一遍像页面在抽搐。

两条通路可以并存：值变化自动播 + 按钮/定时器额外触发，互不排斥。
外观页会在满足自动播放条件时给出绿色提示，
免得作者又去接一条多余的线。
...
用户拖滑动条改进度条数值时发现：**只有停下来后才会有完整动画**，
拖动过程中动画一直被截断。

根因：每次值变化都 `_playToken++` 换 key，
`TweenAnimationBuilder` 从头重建。而拖滑块时值每帧都在变，
动画被反复掐死在第一帧，只有松手后最后那次才播得完。

改为**播放期间不重启**：
* 正在播时只记一笔 `_pendingReplay`，不打断当前这一段
* 这一段播完（`onEnd`）后若有待补，紧接着再播一轮完整的
* 实测拖动 20 帧只产生 2 次起播（拖动中一段 + 松手后补一段），
  旧实现是 20 次

不能直接丢弃待补的变化——那样拖动结束后画面会毫无反馈。

⚠️ `onEnd` 回调发生在构建过程中，直接 setState 会撞上
「widget tree was locked」（与双击断开那次崩溃同源），
因此走 `addPostFrameCallback` 延后。

**连线触发通路故意保留打断行为**：它的触发源是按钮点击这类离散事件，
连点两下本来就该重新播一次，第二次点击不该被忽略。
两条通路的语义不同，已在代码里注明，避免日后被「统一」掉。

测试增至 39 例，新增「连续变值不打断动画」组，
用状态机复刻调度并断言起播次数。

**第二轮反馈：回弹曲线崩溃 + 三种动画不自动播（已修）**

**1. 发光脉冲 + 回弹曲线崩溃**

报错 `Text shadow blur radius should be non-negative`。

根因：`elasticOut` / `bounceOut` 这类曲线**会过冲**——
实测 elasticOut 在输入 0.25 时输出 **1.125**。
`wave = (1.0 - t) / 0.5` 于是变负，
传给 `BoxShadow.blurRadius` 直接触发断言。
默认曲线不过冲，所以之前没暴露。

修法：抽 `waveOf(peak)` 统一夹取到 0~1，
press / flash / glowPulse / ripple / particle 全部改用它。

**数值跳动刻意不夹**：`Transform.scale` 允许过冲，
回弹曲线的弹性观感正来自于此，只兜住负缩放（clamp 0.05~4.0）。

新增回归守卫：遍历全部六条曲线 × 五个采样点，
断言派生量恒在 0~1。以后新增曲线若忘了考虑过冲会立刻失败。

**2. 按压 / 水波 / 粒子在拖 slider 时没播**

不是「效果不明显」，是**根本没播**——我上一步设了 `autoPlayTypes`
白名单，只放行 numberPop / glowPulse / flash。

那个归类（「交互反馈 vs 数值反馈」）是我的主观判断且站不住：
粒子迸发用在数值突变上很自然（扣血炸一下）。
更根本的是，**作者既然特意为这个组件选了某种动画，
就说明他想要那个效果，引擎不该替他否决**。

按用户选定移除白名单，六种全部允许值变化自动播放。
外观页的提示也相应改为不分类型。

测试增至 43 例。

**动画强度偏弱**：用户已反馈但说「这是后话」，留待下一轮一次性调。
自查偏保守的几处：数值跳动峰值仅 +18%（0.3×0.6）、
发光 blur 24 在深色背景上看不出、高亮蒙版上限仅 0.33。
另有结构性问题——几种动画都是单次三角波，
发光脉冲这类语感上更该呼吸两三次。

### A12-3：水波重做为网格折射（方案 C，已完成，待本地测试）

用户对旧水波的评价：**「没有体现让组件被扰动的动荡感，只是用一个
扩大的白圈，感觉很敷衍」**，且「对于数据条来讲，只有中点很小一个
部分有白圈动画」。

用户提出的方向很对：**做一个透明圆环，环内对下方产生放大镜效果**。
真实水波的视觉本质就是折射，不是叠加白圈。

评估了三条路径（A 片元着色器 / B ImageFilter 近似 / C 网格变形），
用户选定**先试方案 C**，不满意再试 A。

**实现**：新增 `services/ui_engine/ripple_mesh_distortion.dart`。
抓取组件纹理 → 切成 32×6 网格 → 按波函数扰动顶点位置（UV 不动）
→ `drawVertices` 绘制。UV 不动是关键，采样区域因此被拉伸 = 折射。

**三个此前算错的地方，都在沙箱里用数值验证纠正了：**

1. **环带剖面撕裂**。最初写 `cos²(πx/2) × sign(x)`，
   在波前处从 -1 跳到 +1，是 **2.0 的阶跃**。
   表现为波前撕裂，且加密网格反而更明显（跳变本身没消失）。
   改用 `-sin(πx)·cos²(πx/2)`，x=0 处连续过零、两侧自然反向。

2. **位移基准取错**。用 `min(w,h)` 时进度条横向位移只有 **0.4px**，
   完全看不出来。改为「该方向上的实际半径」后横向 6~7px、
   纵向 2~3px。粒子动画此前栽在同一个坑（用了 `shortestSide`），
   这也解释了用户说的「只有中点一点点」。

3. **椭圆归一化距离**：`d = √((dx/(w/2))² + (dy/(h/2))²)`，
   组件边界恒为 1.0。扁长组件也能沿长轴扫满，
   根治「只有中点很小一部分」。

**壁面反弹**（用户要求）：波前在 [0,1] 折返行进，
撞到边界原路弹回，全程 2.5 个来回 + `(1-t)^1.5` 衰减，
实测 t=0.4 撞壁、全程 2 次折返。

**本体形变**：横纵**反相**的非等比缩放（同相只是整体缩放，
反相才有挤压回弹的果冻感），上限 9%，结束时精确归位。

**内容冻结的取舍**：`toImageSync` 抓的是静态图。
选择**每帧重抓**，代价是每帧一次离屏渲染，换来内容跟得上
（用户测试场景正是拖 slider 驱动进度条）。
若性能不足可改为抓一次缓存，已留 `refreshTexture` 开关。

新增 `test/ripple_mesh_distortion_test.dart`（18 例）：
剖面连续性、环外归零、壁面反弹、衰减单调、顶点数、
极端 progress 不崩溃、形变反相与归位。

⚠️ **视觉效果无法在沙箱验证**（无 Flutter SDK），
...
**首轮观感反馈：「就是抖动了，没看出有水波」（已调参）**

数值诊断出两个叠加的主因：

**1. 本体形变盖过了折射。**
形变是**全局**运动、折射是**局部**扭曲。首版参数下
200px 宽组件整体横移 **9.2px**，而折射峰值仅 **4.7px**——
眼睛只会注意到整体在动。
形变上限 9% → **3%**，退为辅助；折射强度 0.12 → **0.28**，
峰值升到 10.9px。现在折射是形变的 **6.8 倍**，成为视觉主角。
已加测试锁死这个主次关系。

**2. 环带太宽，笼罩了整个组件。**
`ringWidth=0.6` 时波前居中的影响范围是 **-0.1~1.1**，
整个组件都在环带内，表现为整体缓慢起伏而非一圈环扫过。
收到 **0.4**，波前居中时组件两端已在环带之外，
才有波纹推进的观感。

**连带调整**：窄环带跨越的顶点变少，会被采样成折线。
网格 32×6 → **64×8**（顶点 3072），环带跨约 26 列，
跳变/峰值 0.37。衰减指数 1.5 → 1.2（原来衰减太快，
波还没扫到边缘就没劲了）。

**评估指标也修正了**：此前用「相邻顶点绝对跳变」判断平滑度是错的——
波峰本来就陡，绝对跳变大属正常。正确指标是**跳变/峰值**，
以及环带跨越的列数。

**第二轮观感反馈：仍是抖动（找到真正根因）**

用户补充「文本比进度条更不明显」。这条线索指向了真正的问题——
不是位移量不够，而是**位移方式错了**。

**根因：剖面正负对撞，导致网格折叠。**

前两版剖面都是「内侧正、外侧负」：环内的点向外推、环外的点向内拉。
两股位移在波前处**迎面相撞**，实测相邻顶点间距被压到原来的
**2%**——内容在原地折叠成一团再散开。

那根本不是放大镜，是像素在极小范围里来回挤压，
视觉上当然只剩「抖动」。最大放大率仅 1.58x，且伴随折叠。

真实透镜的光线偏折是**单向连续弯曲**，不是对撞。

**改用单向高斯凸起** `exp(-3x²)`：位移恒为「向外」，
波前处最大、两侧平滑衰减。波前内侧被拉伸（放大）、
外侧被压缩（缩小），这才是透镜扫过的观感。

调整后实测：间距比 **0.49~3.97**，无折叠，
**最大放大率 3.97x**（前一版 1.58x）。

**连带修正两处折叠源：**

1. **只钉死最外圈不够**。内侧第一圈若仍有满幅位移，
   它与钉死点之间会被压扁甚至反向。加边缘渐隐
   （距边界 0.15 内线性衰减到 0）。
2. **clamp 到组件范围本身就是折叠源**。它把多个超界点
   压到同一个边界值上，相邻顶点重合、间距归零。
   改为不 clamp，靠渐隐保证不越界太多。

参数同步：强度 0.28→0.16（单向位移的有效拉伸大得多）、
环带 0.4→0.3、本体形变 3%→**1.5%**（折射已是形变的 5.9 倍）。

**方案 C 放弃（复盘：三轮调参都调在没执行的代码上）**

用户第三轮反馈「看不出任何变化，放弃该方案」。

复盘找到真正原因，**不在数学**：
`RippleDistortionView._capture()` 在 `addPostFrameCallback` 里
只写 `_snapshot` 字段、**从不 setState**。纹理更新后画面永远不重绘，
`CustomPaint` 拿到的始终是 null，一直走 `fallback` 分支显示原图。

也就是说前三轮我反复调的剖面函数、环带宽度、网格密度、
折叠判定——**那些代码根本没被执行到**。用户看到的自始至终是
「原始组件 + 1.5% 本体形变」，那当然只有抖动。

**教训**：视觉效果类改动，沙箱只能验证纯函数的数学正确性，
**验证不了这条链路有没有被真正执行**。
下次做类似功能应先做一个最小可见验证
（比如让它画个纯色块），确认链路通了再堆细节。

已删除 `ripple_mesh_distortion.dart` 与对应测试。
真折射若要重做应上片元着色器（方案 A），它不依赖手工纹理捕获。

**水波回退到可靠的叠加绘制，但修掉旧版三个硬伤：**

| 旧版 | 现在 |
|---|---|
| 单个正圆 + ClipRRect，进度条上只剩中间一条 12px 窄带 | **椭圆**同心环，按组件长宽比拉伸，两端同时被波及 |
| 只有一个圆环 | **三道波错开相位**，一圈接一圈荡开 |
| 单向扩散后消失 | **撞到边界原路弹回**（用户要求） |

外加内侧一道更亮的细边，做出水面反光的层次。

**粒子同步修复同源 bug：**

扩散半径原本用 `size.shortestSide`，进度条上短边是 12，
粒子只飞 14px、占可用横向距离的 **14%**——
正是用户说的「只有中点一点点，看着太小气」。
改用椭圆半径后能飞满 **97%**。

同时补上用户要求的**重力下坠**（随时间平方累积）、
初速度衰减（先快后慢）、粒子大小差异化
（由 index 派生而非 `Random()`——CustomPainter 每帧重新构造，
随机会让粒子逐帧乱跳）。粒子数 10→14，允许飞出组件边界。

**第四轮反馈：不抖了，但「几圈极其扁的椭圆，一点也不像波纹」**

根因：我让椭圆**完全跟随组件长宽比**。
进度条 200×12 时环的长宽比恒为 **16.7:1**——那就是一条扁线。

搞错了一件事：**真实水波是同心圆**。
它在窄容器里之所以特别，是因为圆被上下边界裁掉，
只剩左右两段近乎垂直的弧向两端扫过。
我却直接把环压成了容器的形状，等于把「裁切效果」
错当成「波的形状」提前烘焙进去了。

改为画**正圆**（半径按对角线归一），靠 ClipRRect 自然裁切。
只在长宽比悬殊时做温和纵向压缩，且**压缩比封顶 0.55**，
绝不退化成扁线。

| 组件 | 旧环长宽比 | 新环长宽比 |
|---|---|---|
| 进度条 200×12 | 16.7:1 | **1.73:1** |
| 文本 120×20 | 6.0:1 | **1.60:1** |
| 方块 80×80 | 1.0:1 | 1.00:1 |

**第五轮反馈：粒子量少且从单点爆开；水波像「加了几个同心圆环」**

**粒子（两处调整）**

用户「还是从正中心爆开的，显得量有些少」。

* 发射源从**单点**改为**面区域**：起点在中心附近小椭圆内散开
  （用 `sqrt(h)` 让点在圆面内均匀分布而非聚在中心），
  横向铺得更宽——长条组件的源头本来就该沿长轴展开。
  进度条上源区横向 ±32px，旧版为 0。
* 粒子数 14 → **随面积 25~40**（小图标不必堆满，长条要够密）。
* 角度加入 index 派生的抖动，避免连成规整圆环。

**水波（第三次重做）**

用户「波动环不是透明的，整体感官类似在图像上加了几个同心圆环」。

确实如此：之前是 `alpha 0.8` 的实色描边，那就是圆环贴纸。

改为**渐变光带 + 加色混合**：
* 用 `RadialGradient` 在波前做一圈亮度渐强再渐弱的环，
  两端 alpha 归零，**没有生硬的圆环轮廓**
* `BlendMode.plus` 让波纹与底层内容**相加**而非覆盖——
  亮处更亮、暗处保持，观感是光掠过水面而不是贴纸压在上面
* 纵向压缩改由画布变换实现，渐变本身保持圆形

⚠️ `RadialGradient.stops` 必须严格递增，否则运行时抛异常。
已扫描全时段 × 三道波共 303 组参数，无违规。

至此水波迭代记录（供后续参考，勿重蹈）：

| 版本 | 做法 | 用户反馈 |
|---|---|---|
| 1 | 单正圆 + ClipRRect | 只有中点很小一部分 |
| 2 | 网格折射（方案 C） | 看不出变化——**代码根本没执行** |
| 3 | 椭圆跟随长宽比 | 极其扁的椭圆，不像波纹 |
| 4 | 正圆实色描边 | 像加了几个同心圆环 |
| 5 | 渐变光带 + 加色混合 | 待测 |

### 水波第六版：片元着色器（方案 A，已完成，待本地测试）

用户第六轮反馈「会好很多，不过对于组件渲染的干扰依旧难以观测出来，
很像是两个图层」，并决定「直接开始用方案 A」。

**「像两个图层」不是观感问题，是事实。**
`BlendMode.plus` 只是把两层像素相加，底层内容一个像素都没被移动。
前五版全是 Canvas 叠加绘制，这是它的天花板——
再出第六版叠加方案仍然是两个图层。

**新增文件：**
* `shaders/ripple_refraction.frag` —— 逐像素重采样
* `lib/services/ui_engine/ripple_shader.dart` —— 加载与绘制
* `pubspec.yaml` 增加 `shaders:` 段

**吸取方案 C 的教训。** 那一版失败于「捕获纹理的回调只写字段、
从不 setState」，导致整段变形代码从未执行。这次：
1. 捕获成功后**显式 setState**；
2. 只在动画开始时抓一次纹理，不存在「抓取 → 重绘 → 再抓取」的循环；
3. 着色器不可用时 `RippleShaderView` **原样显示 child**，
   不崩溃也不让组件消失。

**振幅上限用数值扫描定出。** 采样偏移过大会让同侧采样点交叉、
内容折叠成一团。实测 0.14 是临界值，取 **0.12** 留 15% 余量，
对应 200px 宽组件最大偏移 12.2px。

⚠️ 扫描时踩过一个判据错误：**径向位移在中心两侧必然反向**，
那是透镜的固有性质。我最初把跨中心那一步也算作折叠，
连续得到负值，差点把振幅压到毫无效果。
正确判据是「同一侧内部单调」。

⚠️ **GLSL uniform 声明顺序必须与 Dart 侧 setFloat 索引严格一致**，
错位不报错、只会画出莫名其妙的结果。已核对：
`uSize(0,1) uProgress(2) uIntensity(3) uSquash(4) uTint(5~8)`。

新增 `test/ripple_shader_test.dart`（21 例）：
波前折返、剖面非负、采样同侧单调、边缘归零、纵向压缩下限、
衰减单调、加载容错。着色器本身无法在单测运行，
`RippleWaveMath` 是它的 Dart 复刻，两份需同步。
...
**着色器第二轮：有折射了，但卡顿 + 纯色区无感（已修）**

用户「有扰动效果了，不过为什么一顿一顿的？」
「数据条只有两个颜色……只有它们的交接处有明显扰动」。

**1. 卡顿：Opacity(0) 会跳过绘制**

纹理源包在 `Opacity(opacity: 0)` 里，
Flutter 对全透明子树有优化、可能整块跳过绘制，
于是抓到空图或过期内容，表现为一顿一顿。

改为让原始 child **照常绘制**，上层用 `saveLayer` + 不透明覆盖盖住它。
多画一遍的代价远小于一次离屏抓取。
（`Offstage` 同样不行——它连布局都跳过。）

**2. 纯色区无感：这是折射的固有局限**

折射的本质是**采样位置偏移**。进度条内部是大片纯色，
偏移 5px 取到的还是同一个颜色，肉眼看不出任何变化；
只有已填充/未填充的交界处存在颜色梯度，所以只有那一条线在动。

**加强折射强度解决不了**，只会让交界处抖得更厉害。
正确解法是用户提出的那个——**制造新的可动边界**。

新增 `_WavyProgressPainter`：水波期间把进度条填充的右边缘
画成波浪线，两个不同频率的正弦叠加（避免规整正弦太机械），
波幅随衰减包络平息。上限取组件高度的一半，
再大边界会甩出组件、看起来像断裂。

进度满格或波已平息时自动退回直边填充，避免无谓开销。

**记一条通用结论**：几何类特效（折射 / 位移 / 网格变形）
...
**第四轮：卡顿仍在 + 波浪边界看不出（已修两处）**

**1. 卡顿根因：`toImageSync` 按设备像素比全分辨率采样**

它会**同步阻塞等待 GPU 光栅化**。设备像素比常见 2.5~3.0，
每帧抓一张全分辨率图，开销足以让动画掉到几帧一跳。

折射只是把像素搬来搬去、不产生新细节，
半分辨率纹理放大后的模糊完全被波纹扭曲掩盖。
采样比钳制到 **1.0~1.5**。

（`texture()` 用归一化 UV，与纹理实际像素数无关，
降分辨率不会破坏采样映射。）

**2. 波浪边界波幅太小**

原取组件高度的一半。进度条通常只有 12px 高，
折算波幅仅 **3.6px**——叠加在折射之上根本分辨不出来。

改为 1.6 倍高度（12px 的条约 ±11px）。
边界左右摆动本就不受高度约束，真正的限制是不能甩出组件宽度，
已加 clamp 防止填充凭空断开。

**顺带澄清一处误判**：排查时我一度以为纹理冻结在动画首帧，
后来发现 `12ed2a4` 已改为绘制阶段每帧采样，冻结问题不存在。
真正的瓶颈是采样开销而非采样时机。

### 进度条粒子改为边缘喷射（已完成，待本地测试）

用户提议：「粒子动画从进度处向未进度处喷射怎么样？」

**这比中心爆发对得多。** 通用粒子从组件正中心炸开，
表达的是「这个组件有事发生」；而进度条有明确的**数值位置**，
从填充边缘喷射，语义直接变成「数值在这里推进了」。

用户选定的两项参数：
* **范围**：只改进度条。文本、面板没有「值的位置」概念，
  仍用中心爆发。
* **方向**：跟随数值增减——增长向前喷、减少向后喷。
  一律向前会让扣血看起来像回血。

**实现要点：**

必须做成进度条**自己的一部分**（`_EdgeBurstProgressBar`）
而非外层叠加。`_paintAnimationFrame` 只拿得到动画进度
（时间轴 0→1），拿不到数值进度；而 `_buildProgressBar` 在
`_renderModule` 阶段执行、早于所有动画包裹层，
反向传值行不通——波浪边界已经踩过这个坑。

**起喷位置记录变化发生那一刻的边缘**，不是实时进度：
否则连续拖动时喷射点跟着手指跑，看不出是从哪儿溅出来的。

**喷射距离以组件高度为基准**（2.2~4.8 倍），不是宽度。
进度条很扁，用宽度基准会算出 750~960px，粒子直接飞出天际。
现在是 45~69px，占宽度 27~35%。

扇形张角 ±55°：纯直线喷射太机械，全向又失去方向感。
粒子层不裁切（`Clip.none`），允许飞出组件边界。
...
### 动画改为多波次并发叠加（已完成，待本地测试）

用户「每次的动画等待时间太长，要不要弄成在不打断前动画的基础上
重叠播放呢？就不等待动画结束才播放下一条」。

**判断准确。** 之前把「不打断」实现成了**串行排队**：
播放期间的新变化只记一个 bool，等这轮播完再补一轮。
实测 500ms 连续拖动、duration 600ms 时**只播 1 次**，其余全在等。

改为并发后同样条件下播 **6 次**、并发峰值 3 道。

**三处都改了**（它们各自持有动画状态）：

| 位置 | 原方案 | 现方案 |
|---|---|---|
| `ValueChangeAnimator` | 单 token + pending 标记 | 多 `AnimationController` 列表，逐层包裹叠加 |
| `_WavyProgressBar` | `!isAnimating` 才起波 | 时间限流后直接重启 |
| `_EdgeBurstProgressBar` | `isAnimating` 时 return | 多批 `_BurstWave`，各自记录起喷点与方向 |

**为什么波浪边界可以「重启」而粒子必须「多波」：**
波浪是**连续相位**（正弦随 t 推进），从头重启不会跳变；
粒子是**离散**的，重启会让上一批瞬间消失，
所以每批必须各自持有控制器。

**两个共同的约束：**
* 并发上限 3 —— 再多会糊成一团，也白耗性能；超出淘汰最老的一道。
* 最小间隔 90ms —— 拖滑块时值每帧都变，不限流会瞬间堆满上限，
  且各道相位几乎相同，看起来只是一道更粗的波。

`ValueChangeAnimator` 的多波是**逐层包裹**的：
最老的波在最内层。这样效果自然复合——两道数值跳动叠出更大幅度，
两道水波前后错开地扫过，正是「重叠播放」想要的观感。

测试更新为「多波次并发叠加」组（5 例），
覆盖起波次数、并发上限、限流、单次变化、稀疏变化不积压。

### 动画强度调优（已完成，待本地测试）

用户很早提过「有些动画感觉太无感」，此前一直挂着。

**默认 intensity = 0.6 下的实测对照：**

| 动画 | 调整前 | 调整后 |
|---|---|---|
| 短暂高亮 | alpha 仅 **0.33**，几乎看不出闪过 | 0.51 + **外发光** |
| 数值跳动 | 峰值仅 **+18%**，一行数字上察觉不到 | **+33%** + 上浮 3.6px |
| 发光脉冲 | 单层 blur 14.4，深色背景基本不可见 | **三层叠加** + **呼吸两次** |
| 按压凹陷 | 缩到 84% + 变暗 30% | **未改**，本就合理 |

**三处不只是调数值，还改了形态：**

1. **短暂高亮加外发光**——单纯蒙一层色在深色背景上依然不显眼，
   让高亮「溢出」组件轮廓才读得出来。

2. **数值跳动加轻微上浮**（-6px × intensity）——
   纯缩放显得只是在原地胀大，配合抬起才像「弹出来」。

3. **发光脉冲改为呼吸两次**——「脉冲」的语感本就是一下一下的，
   单次三角波起落一回就结束，观感更接近「闪了下」而不是「在发光」。
   同时改用三层阴影（紧贴亮边 6 / 中层光晕 26 / 大范围弥散 54）：
   深色底本来就吃光，只有把亮边收紧、同时把弥散铺开，
   才能同时读出「轮廓在发亮」与「光在扩散」。
   衰减指数 0.8 让第二次呼吸比第一次弱（0.79 → 0.33），
   两次等强会显得机械。

新增「动画强度」测试组（5 例），
断言默认强度下各动画的可辨认阈值、呼吸次数、衰减单调、
以及 intensity=0 时完全静止。

**至此 A12 的六种动画全部调优完毕：**
按压 / 水波（着色器）/ 高亮 / 数值跳动 / 发光脉冲 / 粒子（边缘喷射）。

### 移除 surface 涟漪方案 + 画布级工具栏（已完成，待本地测试）

**1. 移除 `click_to_surface_ripple`**（用户决定）

它是「在 surface 上盖一个扩大的白圈」，与 A12 反复推翻的前几版水波
是同一套敷衍做法。真正的折射水波已由片元着色器实现，
配在**元件自己的外观页**里、用 `event_to_animation` 触发即可，
再单独为 surface 维护一个白圈方案划不来。

老卡里已配的这条连线会被 `isSchemeSelectable` 判非法、
运行端静默跳过，不会崩。

⚠️ `element_animation.dart` 里的 `'click_to_surface_ripple' =>`
映射**必须保留**——那是旧数据迁移路径，
老卡 props 里存的还是这个字符串。

**2. 画布级工具栏**

放在「未选中任何组件」时的左侧，与元件级操作栏**互斥出现**：
同屏两条竖栏会挤掉画布，作者也分不清哪个按钮作用于谁。

| 按钮 | 说明 |
|---|---|
| 撤销 | 此前**完全没有**撤销机制，删除对话框只能写「无法撤销」 |
| 多选删 | 进入后点击=勾选，红框+对勾标记，锁定元件拒绝勾选 |
| 清空页 | 只清当前页，其他页面不动 |

**撤销的实现要点：**

快照打在 `_persistAssemblyElements` 里而**不是逐个操作点**——
所有改动最终都汇到这个函数，逐个改调用点必然漏。

配套三条约束：
* **300ms 去重**：拖动每帧都会 persist，不去重的话撤销一次
  只退回一帧。实测连续拖动 20 帧只留 2 个快照。
* **`_applyingHistory` 标记**：撤销结果要落盘，落盘又会触发 push，
  不挡住的话栈深不变、等于永远撤不回去。
* **栈深上限 40**：角色卡的元素 JSON 可能不小。

批量删除复用单选删除的既有处理：解除组员归属（A14-1c 约束）、
清理孤儿覆写槽位、重建事件监听。

勾选标记加在 `_buildElementWidget` 而非各 body 分支——
body 有原子 / 复合 / 页面路由器 / linker 四种，逐个加必然漏。
...
| 动画 | 最终形态 |
|---|---|
| 按压凹陷 | 缩到 84% + 变暗 30%（未调，本就合理）|
| 水波 | **片元着色器**逐像素折射 + 进度条边界波动 |
| 短暂高亮 | 蒙版 0.51 + 外发光 |
| 数值跳动 | 峰值 +33% + 上浮 |
| 发光脉冲 | 三层阴影 + 呼吸两次 |
| 粒子迸发 | 通用中心爆发；**进度条改为边缘按数值方向喷射** |

**两条驱动通路**：连线触发（`event_to_animation`）与值变化自动播，
可并存，均支持多波次并发叠加。

**未做（判定为锦上添花）：**
* 页面切换动画（A9 的 `base_slide` + fade / 220ms）并入统一通道
  —— 它工作正常且语义不同（页面级 vs 元件级），
  并入的收益不抵改动风险。若日后要做「更高阶页面切换特效」再议。
...
原判「依赖未就绪」是**误判**：`runtimePlacement == 'background'` 机制
早就在 `ui_renderer.dart` 里跑通了（白名单五种原子，标记后运行时
不渲染不接交互），Studio 也有手动切换入口。真正缺的只是
**Assembly 侧的入口**与**拖动手势**。

实现要点：

| 项 | 取值 / 做法 |
|---|---|
| 脱离阈值 | 56px（元件包围盒越过 PCB 内壁的深度） |
| 回吸阈值 | 24px，**必须小于脱离阈值** |
| 越界深度 | 四方向 `max`，用轴对齐包围盒 |
| 吸附行为 | 夹到内壁贴边（`clamp`），不是塞回中心 |
| 反馈 | 脱离瞬间震动 + 顶部提示条 + 落定 SnackBar + 画布「后台」角标 |

**为什么两个阈值必须不等**：相等的话元件会在边界每帧来回抖——
越界一点点被吸回内壁，下一帧手指还在外面又越界。滞回
（hysteresis）就是让「进」和「出」走两条不同的线。

**白名单不动**（用户确认）：`text / switch / progress / indicator / input`，
与 Studio 的 `_canUseBackgroundRuntimePlacement` 逐字一致——
这是跨编辑器契约，两边不一致会出现「Studio 能标 Assembly 不能」。
复合件按产品规则 3.1 永远不许出 PCB，直接排除。

**兜底入口**：元件级操作栏加了「转后台 / 后台中」按钮。
锁定（半锁/全锁）的元件拖不动，只能靠它切换；
从后台移回时会自动 `_snapOffsetInsidePcb`，
否则作者会看到「已移回画布」但屏幕上什么也没多出来。

关键实现：`character_assembly_page/logic.dart` 的
`_pcbOverflowDepth` / `_snapOffsetInsidePcb` /
`_beginBackstageDragTracking` / `_resolveBackstageDragOffset` /
`_commitBackstageDragResult` / `_toggleElementBackstage`。

测试：`test/assembly_backstage_drag_test.dart`（41 例，
含边界抖动 200 帧不翻转、commit 后状态不泄漏到下次拖动两条回归断言）。

### 4.2 叠加页在平级页之间拖放换父 —— ✅ 已完成（原判「不做」被推翻）

#### 原判定为何被推翻

原结论是「手势风险高」：图层面板的 `ReorderableListView` 已占用
长按拖动做同级排序，再加跨父级拖放就得在同一套手势里区分
「排序」与「换父」两种意图。
...
每种页面**只有一种**拖动行为，根本不需要区分意图。
于是按页面类型分流即可：平级页保留 `ReorderableListView`，
叠加页整组换成 `LongPressDraggable` + `DragTarget`。

#### 核心规则：叠加层不能变成子叠加层

**首版实现搞错了方向**，做成了树状嵌套拖放。用户纠正：
换父级指的是**同一类型的父级之间切换**，不是跨级嵌套。
...
这个错误不只在新加的拖放里——**整条链路都建立在错误前提上**：

| 位置 | 旧行为（错） | 新行为 |
|---|---|---|
| `_createPage` | 允许在叠加页上再建叠加页，最深 3 层 | 叠加页恒挂到当前所属的平级页下 |
| `_reparentCandidatesForPage` | depth==2 时只能挂到 depth==1 的叠加页下 | 候选 = 其他平级页 |
| `_canDropPageInto` | 按 depth 分支判定，需防环 | 被拖必须是叠加页、落点必须是平级页 |
| 图层面板 | 叠加页组递归渲染子叠加页 | 不再递归 |

**规则收紧后防环代码可以删掉**：落点恒为平级页，而平级页没有父级，
构不成环。`_isDescendantPage` 因此成为死代码，已删除。

#### 实现要点

| 项 | 做法 |
|---|---|
| 分流依据 | `children.every((p) => p.isOverlay)` |
| 拖起手势 | `LongPressDraggable`，delay 300ms，与原排序手感对齐 |
| 落点 | **只有平级页**（根页与其他平级页） |
| 落点提示 | 可落点绿色描边 0.35 透明，悬停时实色 + 整块染绿 |
| 拖动中源条目 | `Opacity(0.45)` + `childWhenDragging` |
| 反馈 | 震动 + SnackBar「「A」已移入「B」」 |

**判据必须与弹窗共用**：`_canDropPageInto` 与
`_reparentCandidatesForPage` 的规则逐条对齐，
否则会出现「拖放能放进去、弹窗里却查不到这个候选」。
测试里有一组断言专门锁这个等价性。

**换父执行体也共用**：`_applyPageReparent` 同时服务弹窗与拖放。
两份实现迟早漂移——`sortOrder` 重排这种事最容易只改一边。

**`_flattenNestedOverlayPages`**：载入时把嵌套叠加页拍平。
这**不是兼容老卡**，是修正由错误实现产生的非法结构——
留着的话 `_canDropPageInto` 会拒绝一切落点，那些页面卡在原地拖不动。
拍平时保留原本挂靠的平级页（不粗暴全塞进根页），
父级悬空的挂到根页以免丢页面。

**DragTarget 必须包在 Draggable 外层**。反过来的话拖动进行中
Draggable 会重建自身子树，落点判定跟着抖——
这是「手势进行中不能改变自身所在的树结构」的老坑。

弹窗入口（文件夹图标）保留：锁定场景与精确选择仍然用得上。

测试：`test/assembly_page_reparent_drag_test.dart`（32 例），
含「叠加层不能变成子叠加层」组、「拖放判据 == 弹窗候选」等价性组、
以及六条拍平断言。

### 4.3 HUD 显示与交互方案 —— ✅ 数据通道页已重构，其余按需再调

用户盘点后明确：操作栏、顶栏、图层面板**当前都可以**，
唯一碍眼的是数据通道页——「整页都是列排列，而且还有七项，
容易眼花缭乱。在每一条的关系连接上也不大。」

#### 已完成：数据通道页三段式重构

顺序 **数据归属 → 数据标识 → AI 读写**。
先问存放位置，因为它决定后面所有项的含义——存进角色卡设定
和存进会话变量，「名称」的作用完全不同。旧版把名称来源放第一项，
作者得先给一个还不知道用在哪的东西起名字。

| 改动 | 说明 |
|---|---|
| 分三段卡片 | 带序号与标题，浅灰底衬托 |
| 删「玩家可见性」 | **死字段**，全项目零读取点；语义已被「发送给 AI」与「是否在 PCB 上」瓜分 |
| 删「AI 更新应用方式」 | `never` 与「允许 AI 更新 = 不允许」重复 |
| 固定选项平铺 | 3~4 项的一律 `SegmentedField`，不用下拉 |
| 下拉改自绘 | `MenuAnchor` 固定向下展开 + 圆角；原 `DropdownButtonFormField` 弹出方向随可用空间变化，同页九个方向各不相同 |
| 预览挪位 | 从页顶移进段②，紧跟它服务的名称字段 |
| 名称建议 | 右侧按钮拉出状态栏字段列表，**不自动弹出**以免打断输入 |

新增 `lib/pages/character_assembly_page/data_channel_controls.dart`。

#### 未做（按需再调）
- 图层面板样式 / 顶栏布局 / 端口提示样式 —— 用户认为当前可以
- 消息流窗口呈现、scene / extra 的 HUD 细节 —— 跟着实际使用感受调

### 4.3b 状态变化通知系统 —— ✅ 已完成（测试通过）

#### 语义修正：废除「否决权」

现有 `applyPolicy` 三值里的 `confirm` 会弹卡片让玩家**逐条勾选**，
勾掉就不写。用户判断这个语义不对：

> 数据变化本身就不应该让用户可以拒绝，不过像等级升级或者什么，
> 需要告知用户这类重要的变化，就弹窗提醒主角。

**「拒绝」和「知情」是两件事**，要的是后者。因此：

| 旧值 | 归属 |
|---|---|
| `never` | → 「允许 AI 更新 = 不允许」，已重复 |
| `confirm` | → 废除否决权 |
| `auto_low_risk` | → 默认行为 |

新字段 `notifyStyle`: `silent`(默认) / `toast` / `dialog`，
用**三个并排的状态灯**呈现（控件已写好，暂存未入库）。
另加一个自定义文案输入框，占位符 `{name}` `{old}` `{new}`，
留空则用 `等级：3 → 4`。

三处 `applyPolicy` 定义要一起收口：`status_bar_engine.dart`、
`data_channel_prompt_builder.dart`、`data_channel_update_engine.dart`。
`chat_page._confirmStatusChanges` 的勾选框与「应用所选」按钮要拆掉，
改成纯回执。

#### 排队规则（用户定）

```
写入时机：值先写，再通知（通知就是通知，不是预告）

弹窗：堆叠。确认一个露出下一个，逐个查看
浮窗：新的把旧的向上顶，同屏最多 5 个
      满了之后消失一个补一个，不批量放出（避免爆屏）
优先级：弹窗全部确认完 → 才开始生成浮窗
scene 模式：照常走顶部，盖住就盖住
浮窗停留：2.5 秒
```
...
用户判断：状态栏已经设计好了，再给它一套会和 UI 一起跳弹窗，
产生重复性 BUG，意义不大。
...
| 文件 | 内容 |
|---|---|
| `services/ui_engine/status_notification.dart` | `StatusNotifyStyle` / `StatusNotification` / `StatusNotificationQueue` |
| `widgets/status_notification_layer.dart` | 浮窗 + 弹窗展示层，挂在聊天页 Stack 最顶层 |
| `data_channel_controls.dart` | `StatusLampField` 状态灯控件 |

每条浮窗**各持一个定时器**。共用一个的话，后来的浮窗会重置前面那条的
剩余时间，最早那条永远不消失，队列卡死。

弹窗除自定义文案外，副行始终保留「等级 · 3 → 4」——
光写「恭喜升级」看不出数值，而值是先写的，
玩家点掉第一个弹窗时背后的数字可能已经全变完了。

`collectStatusFieldPolicies` 合并多通道时按**强度取较强者**
（dialog > toast > silent）。不能用 `existing ?? item`：
notifyStyle 永远非 null（默认 silent），那样写会让第一个通道的
silent 把后续通道配的 dialog 直接吃掉。

#### 三个连带修复（详见 HANDOFF 3.5c）

做这批时暴露出会话副本同步链上的老问题，一并修了：
`identical` 判不出原地修改、300ms 轮询覆盖外部写入、
消息回滚抹掉开场白确认标记。

#### 遗留的测试
删掉的 `data_channel_form_layout_test.dart` 里有两条有价值的断言
（建议排序规则、选中后光标位置），后续补测试时对着真实实现补回来。

### 4.4 统一几何坐标原则 —— 已转为开发约定，不是待办项
#### 背景
...
#### 当前结论
- 作为长期开发约定保留
- **A14-4 已在遵守**：连线端点、端口朝向、命中检测统一采用
  「局部坐标 → 中心点 → 旋转变换 → 全局偏移」模型，
  相关约束写在 `ASSEMBLY_HANDOFF.md`
- 这不是一条待办，不要再当成未完成项

### 4.5 Binding 状态块名称解析 / 预绑定机制 —— 已由 A13-2 实现
#### 背景
...
#### 当前结论
- **已实现**：见 `lib/models/card_entry_target.dart`（A13-2 三级定位），
  以及数据通道里的 `targetId` / `pendingName` 双轨机制——
  找到字段就绑内部 id，找不到就存预绑定名称，
  进入页面时由 `_realignDataChannelTargets` 补绑。
- 本条自灵感池移出，不再作为待办。

### 4.6 Assembly 资产库底栏 / 横向抽屉方案 —— 已实现，条目过时
#### 背景
...
#### 当前结论
- **已实现**：资产区早已改为底部横向抽屉（`_buildAssetDock`），
  分类切换、长按竖向拖出生成、与 HUD 的避让都已落地。
- 本条自灵感池移出，下方内容仅作为当初的设计记录保留。
