...
| A10 | mode 差异逻辑收口 | A10-0/1/2 完成，测试通过 | 🚧 |
| A11 | 消息流窗口 | 未开始 | ⏳ |
| A12 | 高级动画 | 未开始 | ⏳ |
...
A10-3  extra_companion（消息下挂 + 滚动跟随）
A10-4  opening（全屏 + 确认销毁）
A10-5  scene（全屏接管，最后做）
```
...
#### A10-2.5：组件语义标记机制（已完成，待本地测试）
A10-4 的「确认后销毁」与 A10-5 的「退出场景」都需要作者能指定
哪个按钮承担该职责。三处各做一次不如先做一套通用能力，故提前到此。

新增 `lib/services/ui_engine/ui_semantic_role.dart`。

设计原则（延续 `is_overlay_container` 的思路）：
- **不新增专用组件类型**。作者用普通 button 表达意图，
  外观完全自定，引擎只识别职责。
- 标记存在 `module.properties['semanticRole']`，随实例保存，
  不回写资产库模板。
- 各 mode 只认自己需要的角色，不认识的安全忽略。

已接入的角色：
- `dismiss` 关闭 / 收起 —— 常驻折叠成球、开场白关闭、场景退出
- `confirm` 确认并关闭 —— 语义比 dismiss 更明确，供 opening 使用

运行时接线：
- `UIAssemblyRuntimeView` 新增 `onDismissRequested`，
  复用 button 已有的 `tap` 脉冲识别角色，**不改渲染器**——
  被标记的按钮仍可正常参与 linker 等既有配置。
- `ChatAssemblyMount` 透传该回调，并提供 `hasSemanticRole()`
  供调用方判断是否还需要内置兜底按钮。

兜底策略（重要）：
- 作者标了 `dismiss`/`confirm` → 隐藏内置折叠按钮，用作者的。
- 作者没标 → 保留内置按钮。**不能两者都没有**，
  否则用户失去唯一操作入口。

编辑器：
- 原子实例编辑器新增「运行时职责」下拉，附带各角色的效果说明。
- 画布上显示紫色徽标，作者一眼能看出哪个按钮绑了职责。

暂缓项说明：
- `drag_handle`（作者自定义拖动把手）常量已定义但**不在下拉选项里**。
  它需要把拖动手势下沉到渲染层，并解决与内部 slider 的手势竞争，
  属于独立一步。列出但不生效会让作者以为标了就有用，故先不开放。
  内置把手继续保留，保证挂件始终可移动。

测试：`test/ui_semantic_role_test.dart` 覆盖读取、非法值回落、
复合组件内部查找、兜底判断、以及「未接入的角色不出现在选项里」。
