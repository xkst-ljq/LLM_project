...
#### A14-3 第二步：外观专项页（已完成，待本地测试）

**先纠正一个判断失误。** 上一轮我把「实例编辑器分文件」判为
过度设计，理由是「Assembly 该管的参数不多」。
用户追问「不做的话 surface 怎么改颜色」——核实后发现：

**Assembly 里根本没有颜色编辑，一个取色器都没有。**
surface 只能改圆角与透明度，颜色 / 材质 / 形状全改不了；
进度条只能改 min/max/current，轨道色改不了。
作者拖一个面板进来，它永远是资产模板里那个颜色。

失误原因：**把分工线用过头了**。分工说的是
「组件的**行为规则**归 Studio」（如 indicator 的状态映射引擎），
而颜色、形状属于「这一个实例在这张卡里长什么样」——
按我们自己立的口诀，那是 Assembly 该管的。
同一个面板在 A 卡深蓝、B 卡暖橙，完全正常。

用户补充的场景很有说服力：**叠加页拿 surface 当页面容器**，
外观受限的话设计感起不来。

**形态：统一一个「外观」页**（用户选择），从实例编辑器进入。
不做「每类组件一页」——共性字段（主色 / 圆角 / 透明度）人人都有，
拆十几个页面反而让作者记不住入口。
...
|---|---|
| 通用 | 主色、圆角、透明度 |
| surface / base_box | 材质（毛玻璃/纯色/渐变/描边）、形状（含心形、星形）|
| progress | 进度条形状（含环形/心形）、轨道底色 |
| slider | 手柄形状 |
| input / select | 外框样式、占位文字色、输入文字色 |
| message_flow | 玩家/角色气泡色、气泡圆角 |
| indicator | 兜底底色（**状态规则仍只在 Studio**）|

用固定色板而非全色域取色器：作者要的是几个协调的颜色，
全色域容易挑出一堆相近色，小屏上也难精确操作。

**又一次踩到「双写覆盖」陷阱**：
实例编辑器保存时原本会回写 `borderRadius` / `opacity`，
而外观页保存后这些 controller 仍是打开对话框那一刻的旧值——
会把刚调好的外观覆盖掉。已移除回写，与数据通道同样处理。

#### A14-3 第三步：button 回归纯热区 + 连线级手势（已完成，待本地测试）

用户两个提问牵出同一批历史遗留。

**其一：button 为什么有外观编辑？**

我最初答「button 是可见组件，给外观合理」——**答错了**。
核对 `_buildButton` 结尾才发现，运行期它返回的是
`SizedBox.expand()`，即**完全不显形的纯热区**；
只有 `showTextOnRuntime` 为真才画出文字。
而那个开关又是实例编辑器按「文字是否非空」自动置位的。

也就是说，button 的外观三项（主色/圆角/透明度）
在绝大多数情况下对运行时零影响，纯粹误导作者；
其中圆角更是彻底的假开关——渲染端写死 `circular(4)`。

**其二：button 本就该是「可触发区域」，为什么还要联动 surface？**

用户点破：正因为 button 看不见，才需要 surface 代它做按压反馈。
`text` / `showTextOnRuntime` 是 Studio 早期缺编辑入口时留下的设定，
「不影响就留下了」，现在予以废弃。
...
| 项 | 处置 |
|---|---|
| 运行期渲染 | 恒为 `SizedBox.expand()` 热区，删 `showTextOnRuntime` 分支 |
| 编辑期渲染 | 统一灰色区域 + `touch_app` 图标，表明热区身份（不再用 module.color）|
| 外观入口 | button 移出 `_supportsAppearanceEditor` |
| `text` / `showTextOnRuntime` / `active_gesture` | 两侧编辑器保存时主动 remove，打开一次即完成迁移 |
| 资产模板 | 不再预置 `'text': ''` |

**其三：`active_gesture` 的去向。**

用户说明其原始设想：在按钮上选一种触控方式，
**隐藏其他方式的连线**，以此实现「同一按钮不同触控触发不同结果」。
但后来连接方案里又加了触发方式选择，两者重复，这个方案遂被架空。

确认它是**死代码**——全仓只有那个下拉自己写入，无人读取。
作者在 Studio 选「长按触发」，实际行为仍是单击。

**改为连线级手势**，这比原设想更彻底：不需要隐藏任何连线，
同一个按钮挂三条连线，单击 / 双击 / 长按各接各的目标，同时生效。

落点是连线的 `sourcePort`——运行端 `LinkerService.initEventBusListener`
本就按 `'tap'` / `'double_tap'` / `'long_press'` 三个端口名分派事件
（`linker_service.dart:271-297`），**运行端一行都不用改**。

**刻意不为「双击触发某某」单开方案 id**：那会让方案矩阵翻三倍，
且仍然做不到「同一按钮三种手势并存」。
`_schemeSourcePort` 里预留的 `double_click_` / `long_press_`
前缀分支因此继续闲置（矩阵中无此类方案）。

覆写的守卫条件：**来源类型是 button 且方案推导端口为 `tap`**。
避免哪天给 button 加值型方案时被手势串改端口。

手势对所有 button 方案开放（用户选择）。
surface 按压 / 涟漪选非单击时给橙色提示——
这两条走 `tap_down` 即时触发，改双击/长按要等判定结束，手感会变迟钝。

**手感参数**（`doubleTapIntervalMs` / `longPressThresholdMs`）
放实例编辑器，因为它们属于这个按钮的调校而非某条连线；
且**仅在该按钮确实有非单击连线时才显示**，
否则平白多两个数字框、作者还得猜用途。
保存时就地 clamp 到渲染端认可的区间（100~1000 / 150~3000），
让存进卡里的值就是生效值。

**Studio 侧一并清理**（原计划「A14 全做完再动 Studio」，
但这两处是同一批遗留、且属于删除而非改造，风险低于放着不管）：
按钮编辑器的文案输入框、手势模式下拉、运行期显字开关全部移除，
换成一段说明「按钮是热区 / 反馈请连 surface / 手势在连线里选」。

新增 `test/assembly_button_gesture_test.dart`（15 例）：
手势覆写与守卫、三手势端口互异、显示条件、属性迁移与 clamp。
`assembly_appearance_test.dart` 中 button 移入「无外观」组。

**A14-3 剩余：按组件类型分文件 —— 暂不做**

外观与数据通道这两块「真正臃肿」的都已独立成页，
实例编辑器只剩名称 + 内容字段 + 标记 + 两个入口按钮。
再按类型拆文件属于过度设计，改为**按需拆**。
