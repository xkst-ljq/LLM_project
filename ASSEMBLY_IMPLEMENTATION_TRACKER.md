...
| A11 | 消息流窗口 | 富文本 / 着色已通过；消息操作 + 头像待测 | 🚧 |
| A12 | 高级动画 | 未开始 | ⏳ |
| A13 | 玩家参与的会话初始化 | A13-1/2/3 完成，待本地测试 | 🚧 |
...
### A13-2：结构化档案 = 直接指向角色卡条目（已完成，待本地测试）
...
### A13-3 改版：配额分配（`pool_to_allocation`，已完成，待本地测试）
...
| 维度 | 取值 |
|---|---|
| 来源（池） | text / progress / slider / input / math_node |
| 目标（分配项） | slider / progress |
