...
const Walkthrough _uiStudio = Walkthrough(
  ...
  steps: [
    ...
    WalkthroughStep(
      title: '联动协议分组一览',
      body: '68 条联动协议按**用途**分成几组，接线时按「想让界面发生什么」来找对应的组：\n\n'
          '① 点击触发：按钮点击 → 让某个东西发生（动画/开关/清空/重置/启停/计算）\n'
          '② 状态控制：开关/颜色 → 控制别的组件的显隐、使能、锁定、冻结\n'
          '③ 数值搬运：滑块/进度/输入/文本 → 把数值喂给进度条、滑块、文本、计算\n'
          '④ 文字渲染：各种值 → 变成文本显示出来\n'
          '⑤ 条件判断：非空/匹配/阈值/校验 → 决定按钮可不可点、开关开不开\n'
          '⑥ 定时驱动：定时器 → 持续翻转开关、加进度、触发计算\n'
          '⑦ 数值计算：计算节点 → 运算结果驱动文本/进度，或注入另一计算\n'
          '⑧ 联动全局：配额分配、求和汇总\n\n'
          '下面逐组讲清「用途 + 用法」。',
      imageHint: '协议分组总览',
    ),
    WalkthroughStep(
      ...
      body: '用途：把「按钮被点了」这个动作接到某个目标上。\n\n'
          ...
          '• 定时器 → 启动/停止切换、或重置归零\n'
          '• 计算节点 → 手动触发立即算一次\n\n'
          '按钮本身是透明热区，常盖在面板上做成「点这个区域触发」。',
      imageHint: '按钮点击协议',
    ),
    WalkthroughStep(
      title: '② 状态控制（switch / indicator）',
      body: '用途：用一个「开/关」或「颜色状态」去控制别的组件。\n\n'
          '用法：开关或指示灯连到任意组件：\n'
          '• 显隐：开显示、关隐藏\n'
          '• 使能：开可用、关禁用并淡化\n'
          '• 锁定：开锁住不能编辑、关解锁\n'
          ...
          '• 文本：开显示一段、关显示另一段\n'
          '• 定时器：开才运行、关停止\n\n'
          '典型场景：「高级选项」开关，开了才展开某块区域。',
      imageHint: '开关/指示灯状态控制',
    ),
    WalkthroughStep(
      title: '③ 数值搬运（slider / progress / input / text）',
      body: '用途：把一个组件的数值搬到另一个组件上显示或控制。\n\n'
          '用法：\n'
          '• 滑块 → 进度条 / 文本：拖滑块，进度条跟着走\n'
          '• 滑块 → 计算节点参数：松手才把值注入\n'
          '• 进度条 → 文本：显示当前值 / 最大 / 百分比\n'
          '• 输入框 → 进度条 / 滑块：输入数字驱动\n'
          '• 文本 → 计算节点：从文本里取数（纯数值 / 第一个数 / 第 N 个 / 关键字）\n\n'
          '典型场景：把滑块值显示成文字「HP 65/100」。',
      imageHint: '数值搬运',
    ),
    WalkthroughStep(
      title: '④ 文字渲染（→ text）',
      body: '用途：把各种值实时变成界面上的文字。\n\n'
          '用法：\n'
          '• 滑块 / 进度 / 下拉 / 定时器 → 文本：值变化文本就更新\n'
          '• 输入框 → 文本：实时同步、或提交（回车/失焦）后同步、或提交后清空\n'
          '• 开关 → 文本：开显示一段文案、关显示另一段\n'
          '• 计算节点结果 → 文本：结果按模板渲染，或按真假显示条件文案\n'
          '• 下拉 → 文本：选中的项显示出来\n\n'
          '文本配合模板（如 {{current}}、{{value}}、{{total}}）就能拼出完整句子。',
      imageHint: '文字渲染',
    ),
    WalkthroughStep(
      title: '⑤ 条件判断（→ button / switch / indicator）',
      body: '用途：根据输入的内容或数值，决定按钮可不可点、开关开不开、指示灯变什么色。\n\n'
          '用法：\n'
          '• 输入框 → 按钮：非空才启用 / 通过校验才启用\n'
          '• 输入框 → 指示灯：按校验状态（空/有效/无效）或长度变色\n'
          '• 文本 → 按钮：非空 / 匹配到才启用\n'
          '• 文本 → 开关：匹配才开\n'
          '• 进度 → 按钮 / 开关：达到阈值才启用 / 开启\n'
          '• 输入 → 下拉：输入匹配某选项自动选中、或实时过滤选项\n\n'
          '典型场景：没填完不让点「提交」，填对了按钮才亮。',
      imageHint: '条件判断',
    ),
    WalkthroughStep(
      title: '⑥ 定时驱动（timer）',
      body: '用途：让某个变化**自动持续发生**，不需要玩家操作。\n\n'
          '用法：定时器连到：\n'
          '• 开关 → 每到一次 Tick 翻转 / 强制开 / 强制关\n'
          '• 进度条 → 每次 Tick 加一步 / 减一步\n'
          '• 计算节点 → 每次 Tick 触发重算\n'
          '• 文本 → 显示当前值 / Tick 次数 / 步长\n\n'
          '典型场景：血量随时间自动回复的进度条、倒计时。',
      imageHint: '定时驱动',
    ),
    WalkthroughStep(
      title: '⑦ 数值计算（math_node）',
      body: '用途：做多步数值运算，把结果接到显示或其它计算。\n\n'
          '用法：\n'
          '• 滑块 / 进度 / 文本 / 其它计算 → 计算节点：注入各参数口\n'
          '• 计算节点 → 文本 / 进度条：结果驱动显示\n'
          '• 计算节点 → 另一计算节点：串联多步运算\n'
          '• 布尔结果 → 文本：显示条件文案；→ 进度条：true 满 / false 空\n\n'
          '典型场景：把两个属性加起来算个总战力再显示。',
      imageHint: '数值计算',
    ),
    WalkthroughStep(
      title: '⑧ 联动全局：配额分配 / 求和',
      body: '两个把「多个组件当整体」的协议：\n\n'
          '• 配额分配（pool → allocation）：把一个组件当作「可分配总量」，其它组件（滑块/输入框）从中分配点数。总量写「10」，连到力量/敏捷/智力三个滑块，即做成属性点分配面板；分配组件自动归零并被限制在剩余额度内\n'
          '• 数值求和（sum → display）：把多个数值加起来显示到一处，可配模板「已分配 {{value}}/{{total}}，剩余 {{remain}}」\n\n'
