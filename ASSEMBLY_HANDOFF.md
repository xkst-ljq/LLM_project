...
剩余：A10-3 伴生 / A10-4 开场白 / A10-5 场景。
A10-1 的挂载基础设施可直接复用，主要工作是各自的形态差异。

### 待后期统一处理的优化（用户已提出，暂缓）
1. **按住 PCB 空白处直接拖动挂件**（取代拖动把手）。
   暂缓原因：会与 slider 的抢占式水平拖动识别器冲突，
   需要重新设计整套手势仲裁；且「哪些区域算空白可拖」还没有定义。
   建议等 A10-3/4/5 完成、四种 mode 的交互需求明确后统一设计。
2. ~~**用 button 语义标记实现折叠 / 退出 / 确认**~~ —— 已在 A10-2.5 完成，
   见 `ui_semantic_role.dart`。新增 mode 时直接复用 `onDismissRequested`。
   注意保留内置兜底按钮：作者没标记时用户不能失去唯一操作入口。
   （原记录）用 button 语义标记实现折叠 / 退出 / 确认（取代硬编码按钮）。
