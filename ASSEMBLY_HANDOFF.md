...
### 下一步建议
- 先做 A7.5 / A5-0：Assembly 资产区最小补全
- 评估底部资产栏 / 横向抽屉方案
- 至少开放 `button / linker / page_router`
- 再进入 A5 Assembly 内 linker 连线，打通 `button → linker → page_router`
...
### 3.6 页面路由器测试入口规则
- 当前点击 `page_router` 本体跳转，只是 A7 编辑态测试入口。
- 最终运行时不应要求用户点击逻辑节点本体。
- 后续应通过 `button → linker → page_router` 触发页面切换。
- `page_router` 本体长期应作为编辑器可见、运行时隐藏的后台逻辑节点。

### 3.7 资产库底栏 / 横向抽屉方案
- 资产库后续建议移动到底侧，分为：逻辑组件 / 基础交互 / 基础显示 / 复合组件。
- 点击分类标题后弹出横向抽屉，可左右滑动；点击其他分类切换，点击同分类或画布空白关闭。
- 底部横向抽屉不应遮住顶部标题栏。
- 拖出生成逻辑应从“水平位移阈值”改为“竖直向上位移阈值”，避免和横向滑动浏览冲突。
- 右下角 HUD 会与底部抽屉冲突；候选方案是抽屉展开时 HUD 上移、折叠，或并入底栏后续统一设计。
- 该方案适合作为 A7.5 / A5-0 资产区最小补全的设计候选。
...
> 继续 `ASSEMBLY_HANDOFF.md` 的上下文，当前分支是 `arena/019f9cee-llm-project`，A2/A6/A3-1/A3-2/A3-3/A3-4/A7 基础版已完成，A7 待本地测试。通过后继续 A4 暴露端口体验增强或 A5 Assembly 内 linker 连线。请遵守每一步完成后 commit/push、非阻塞问题并入下一步处理的节奏。
