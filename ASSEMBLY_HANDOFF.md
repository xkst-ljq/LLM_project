...
> **例外说明**：上表约束的是 App 内**编辑器手工装配**（Studio 造零件 vs Assembly 装机器）。
> **转译器的确定性编译器**（`ui_assembly_builder.dart`）属**程序化装配**——它一直就在设置主题色 /
> 圆角 / 材质（`material` / `shape` 枚举下标），等价于 Studio 的模板构建，不受「Assembly ❌ 外观」限制。
> 因此编译器为还原原卡 CSS 而设置的渐变材质（`gradient`）、描边（`outline`）、发光（`glow` → `__anim` /
> `__staticGlow`）是合法装配行为，不要误读上表而回退它们。
