...
- **冲突（B）**：交由后续转换工具（005-B AI 智能归类 + 005-C 精修对照）处理，
  ...
  - **占位符 `{{char}}/{{user}}`**：不在转换阶段处理。引擎在发送时统一渲染整个
    system prompt（description / 条目 / 世界书 / 摘要均覆盖），与酒馆一致，
    手写卡与导入卡都原生支持。
  - **description 常驻位**：规则阶段**不**把 description 塞进常驻位（正则无法
    区分性格/场景/背景，硬塞会污染）。该工作属于第二步 AI 归类。待 005-B 完成后，
    可把"常驻兜底"作为 AI 提取不全时的保险再评估，当前不做。
