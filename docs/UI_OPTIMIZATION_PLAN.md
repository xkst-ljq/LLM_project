...
- 选中角色信息浮层重设计为**三块贴边玻璃抽屉**（名字贴左上、标签贴右上、
  介绍贴左下），与卡片拐角重合的一角用卡片圆角(16)、其余直角贴边；玻璃紫色调
  （亮主题深紫/暗主题浅紫），玻璃上文字色反转（深紫玻璃→白字/浅紫玻璃→深字）、
  介绍高对比；展开动画从边上展开（无外部位移），带开/关/换选动画。
...
- 封面中间升级为**词云**：统计所有条目标题 + 主关键词的出现频率，
  每个词用**圆形气泡**裹住、气泡越大语义越重（最多 7 个词），按**正六边形
  蜂窝**排列（语义最重的在中心，其余按圈向外），气泡内部字号随气泡大小
  变化，并带**随机浮动**动画（各自独立相位/幅度/频率，缓慢错落浮动）；
  用 `Expanded` 占满中间空间、`FittedBox` 等比缩放，不溢出到下方信息栏；
  无词汇时回退为首字标识。
